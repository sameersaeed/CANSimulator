...
OBDClient::OBDClient(const std::string& iface) : m_socket(iface) { 
    m_socket.set_filter(0x7E8, 0x7FF);
}

std::optional<QueryResult> OBDClient::query(PID pid, int timeout_ms) {
    can_frame request{};
    ...
    if (!m_socket.send_frame(request)) return std::nullopt;

    // discard stale frames until a matching response is found
    while (true) {
        auto frame = m_socket.receive_frame(timeout_ms);
        ...
        auto decoded = decode_response(frame->data[2], frame->data[3], frame->data[4]);
        if (!decoded) return std::nullopt;

        return QueryResult{decoded->name, decoded->value, decoded->unit, latency_us};
    }
}
...
void OBDClient::benchmark(PID pid, int count) {
    ...
    double total_s    = std::accumulate(latencies.begin(), latencies.end(), int64_t{0}) / 1e6;
    double throughput = (total_s > 0) ? success / total_s : 0.0;
    ...
    std::cout << "\n=== CANSimulator Benchmark: " << pid_to_name(pid)
              << " x" << count << " ===\n"
              << std::fixed << std::setprecision(3)
              << "  Successful:   " << success << "/" << count << "\n"
              << "  Avg latency:  " << avg_us / 1000.0 << " ms\n"
