...
Dashboard::Dashboard(VehicleState& state, Metrics& metrics) : m_state(state), m_metrics(metrics) {
    ...
    m_window = WindowPtr(SDL_CreateWindow("CANSimulator - OBD-II Dashboard", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
        WIN_W, WIN_H, SDL_WINDOW_SHOWN));
    if (!m_window) throw std::runtime_error(std::string("CreateWindow: ") + SDL_GetError());

    m_renderer = RendererPtr(SDL_CreateRenderer(m_window.get(), -1, SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC));
    if (!m_renderer.get()) throw std::runtime_error(std::string("CreateRenderer: ") + SDL_GetError());

    SDL_SetRenderDrawBlendMode(m_renderer.get(), SDL_BLENDMODE_BLEND);

    m_fontLg = FontPtr(tryLoadFont(40));
    m_fontMd = FontPtr(tryLoadFont(17));
    m_fontSm = FontPtr(tryLoadFont(13));
...
void Dashboard::drawGauge(int cx, int cy, int r, double value, double maxVal, const char* label, const char* unit) {
    ...
    drawArc(m_renderer.get(), cx, cy, r, ARC_START, ARC_START + ARC_SWEEP, DARK_ARC, 9);
    ...
    if (frac > 0.001) {
        ...
        drawArc(m_renderer.get(), cx, cy, r, ARC_START, ARC_START + frac * ARC_SWEEP, arcCol, 9);
    }
    ...
    for (int i = 0; i <= 20; ++i) {
        ...
        setCol(m_renderer.get(), major ? TEXT : BORDER);
        SDL_RenderDrawLine(m_renderer.get(),
    ...
    for (double lf : labelFracs) {
        ...
        blitText(formatLabel(lval, true), lx, ly - 7, m_fontSm.get(), SUBTEXT);
    }

    // needle
    drawNeedle(m_renderer.get(), cx, cy, r - 16, ARC_START + frac * ARC_SWEEP, COL_WHITE);

    // center cap
    drawFilledCircle(m_renderer.get(), cx, cy, 14, PANEL);
    drawFilledCircle(m_renderer.get(), cx, cy,  7, COL_WHITE);

    // large val in center
    bool isRPM = (strcmp(label, Labels::ENGINE_RPM) == 0); // show full value for RPM
    blitText(formatLabel(value, !isRPM), cx, cy + 48, m_fontLg.get(), TEXT);
    blitText(unit,  cx, cy + 95,  m_fontSm.get(), SUBTEXT);
    blitText(label, cx, cy - 88, m_fontSm.get(), SUBTEXT);
}

void Dashboard::drawStat(int x, int y, int w, int h, const char* label, const std::string& value, SDL_Color vcol) {
    SDL_Rect bg = {x, y, w, h};
    
    setCol(m_renderer.get(), PANEL);
    SDL_RenderFillRect(m_renderer.get(), &bg);
    
    setCol(m_renderer.get(), BORDER);
    SDL_RenderDrawRect(m_renderer.get(), &bg);

    blitText(label, x + w / 2, y + 8,        m_fontSm.get(), SUBTEXT);
    blitText(value, x + w / 2, y + h / 2 + 5, m_fontMd.get(), vcol);
}
...
void Dashboard::drawKeyHint(int x, int y, const char* keys, const char* action, bool active) {
    ...
    setCol(m_renderer.get(), active ? COL_BLUE : PANEL);
    SDL_RenderFillRect(m_renderer.get(), &bg);
    
    setCol(m_renderer.get(), active ? COL_BLUE : BORDER);
    SDL_RenderDrawRect(m_renderer.get(), &bg);

    blitText(std::string(keys) + "  " + action, x + w / 2, y + h / 2 - 6, m_fontSm.get(), active ? COL_WHITE : SUBTEXT);
}
...
void Dashboard::render() {
    ...
    // clear
    setCol(m_renderer.get(), BG);
    SDL_RenderClear(m_renderer.get());

    // header bar
    SDL_Rect hdr = {0, 0, WIN_W, 42};
    setCol(m_renderer.get(), PANEL);
    SDL_RenderFillRect(m_renderer.get(), &hdr);
    setCol(m_renderer.get(), BORDER);
    SDL_RenderDrawLine(m_renderer.get(), 0, 42, WIN_W, 42);

    blitText("CANSimulator",   WIN_W / 2, 5,  m_fontMd.get(), TEXT);
    blitText("OBD-II | vcan0", WIN_W / 2, 24, m_fontSm.get(), SUBTEXT);

    // live CAN request counter 
    std::string CANReqCtr = "CAN req: " + std::to_string(reqs);

    int rw = 0;
    TTF_SizeText(m_fontSm.get(), CANReqCtr.c_str(), &rw, nullptr);
    blitText(CANReqCtr, WIN_W - rw - 18, 14, m_fontSm.get(), SUBTEXT, false);

    // center divider
    setCol(m_renderer.get(), BORDER);
    SDL_RenderDrawLine(m_renderer.get(), 450, 50, 450, 448);
    ...
    // throttle % 
    blitText("throttle  " + formatLabel(m_throttle, false) + "%", SPD_CX, SPD_CY + 118, m_fontSm.get(), SUBTEXT);
    ...
    blitText("Q / ESC  quit", 520, ky + 8, m_fontSm.get(), SUBTEXT, false);
